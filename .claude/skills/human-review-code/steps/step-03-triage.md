...
1. **Normalize** findings into a common format. Expected input formats:
   ...
   - Performance & Efficiency Hunter: markdown list with `category`, `title`, `detail`, `suggestion` fields
   ...
   - `source` -- `blind`, `edge`, `auditor`, `perf`, or merged sources (e.g., `blind+edge`)
   - `title` -- one-line summary
   - `detail` -- full description
   - `location` -- file and line reference (if available)
   - `category` -- for perf findings: `n_plus_1`, `inefficient_loop`, `missing_bulk`, `oversized_component`, `unnecessary_api`, `inefficient_transform`, `complexity`, or `general` for other sources
...
2. **Deduplicate.** If two or more findings describe the same issue, merge them into one:
   ...
   - Set `source` to the merged sources (e.g., `blind+perf`).
...
3. **Classify** each finding into exactly one bucket:
   ...
   - **performance** -- Performance-related issue (N+1, inefficient loop, oversized component, etc.). Treated as `patch` but grouped separately for visibility.
